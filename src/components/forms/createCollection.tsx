  ...
  return (
    <form onSubmit={handleSubmit(createCollection)} className="form flex w-full relative">
      <input
        ...
        className="bg-gray-200 text-title h-11 font-medium w-full max-w-3xl py-3 px-5 rounded-3xl" 
        {...register("name")}
      />
      <button className="absolute right-0 h-11 shadow-2xl bg-linear-to-tl from-primary to-primary-hover text-white font-semibold py-2 px-6 rounded-3xl">
        Criar
      </button>
    </form>
  );
